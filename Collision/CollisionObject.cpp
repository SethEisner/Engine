...
CollisionObject::CollisionObject(GameObject* game_object, Mesh* mesh) : 
	...
	for (size_t i = 0; i != m_obb_count; ++i) {
		...
		m_oriented_boxes[i].m_oriented_box.CreateFromPoints(m_oriented_boxes[i].m_oriented_box,
			vertex_count, reinterpret_cast<DirectX::XMFLOAT3*>(vertex_buffer + base_vertex + start_index), mesh->m_vertex_stride);
	...
	for (size_t i = 0; i != 8; ++i) { // transform each corner into m_box's/AABB's space
		DirectX::XMStoreFloat3(corners + i, DirectX::XMVector3Transform(XMLoadFloat3(corners + i), XMLoadFloat4x4(&m_oriented_boxes[0].m_transform)));
	}
