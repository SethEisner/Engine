...
bool Renderer::init() {
	assert_if_failed(CreateDXGIFactory1(IID_PPV_ARGS(&m_dxgi_factory)));
	...
	assert_if_failed(D3D12CreateDevice(nullptr, D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&m_d3d_device)));
	assert_if_failed(m_d3d_device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_fence))); // fence is for syncronizing the CPU and GPU, will remove later
	...
	assert_if_failed(m_d3d_device->CheckFeatureSupport(D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS, &ms_quality_levels, sizeof(ms_quality_levels)));
...
void Renderer::on_resize() {
	...
	flush_command_queue();
	assert_if_failed(m_command_list->Reset(m_command_list_allocator.Get(), nullptr));
	...
	assert_if_failed(m_swap_chain->ResizeBuffers(SWAP_CHAIN_BUFFER_COUNT, engine->get_window()->get_window_width(), engine->get_window()->get_window_height(), m_back_buffer_format, DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH));
	m_current_back_buffer = 0;
	CD3DX12_CPU_DESCRIPTOR_HANDLE rtv_heap_handle(m_rtv_heap->GetCPUDescriptorHandleForHeapStart());
	for (size_t i = 0; i != SWAP_CHAIN_BUFFER_COUNT; ++i) {
		assert_if_failed(m_swap_chain->GetBuffer(i, IID_PPV_ARGS(&m_swap_chain_buffer[i])));
	...
	depth_stencil_desc.Width = engine->get_window()->get_window_width();
	depth_stencil_desc.Height = engine->get_window()->get_window_height();
	...
	assert_if_failed(m_d3d_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE, 
	...
	assert_if_failed(m_command_list->Close());
	...
	m_screen_viewport.Width = static_cast<float>(engine->get_window()->get_window_width());
	m_screen_viewport.Height = static_cast<float>(engine->get_window()->get_window_height());
...
void Renderer::create_command_objects() {
	...
	assert_if_failed(m_d3d_device->CreateCommandQueue(&command_queue_desc, IID_PPV_ARGS(&m_command_queue)));
	assert_if_failed(m_d3d_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(m_command_list_allocator.GetAddressOf())));
	assert_if_failed(m_d3d_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_command_list_allocator.Get(), nullptr, IID_PPV_ARGS(m_command_list.GetAddressOf())));
	m_command_list->Close();
}
void Renderer::create_swap_chain() {
	m_swap_chain.Reset();
	DXGI_SWAP_CHAIN_DESC swap_chain_desc;
	swap_chain_desc.BufferDesc.Width = engine->get_window()->get_window_width();
	swap_chain_desc.BufferDesc.Height = engine->get_window()->get_window_height();
	swap_chain_desc.BufferDesc.RefreshRate.Numerator = 60;
	swap_chain_desc.BufferDesc.RefreshRate.Denominator = 1;
	swap_chain_desc.BufferDesc.Format = m_back_buffer_format;
	swap_chain_desc.BufferDesc.ScanlineOrdering = DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED;
	swap_chain_desc.BufferDesc.Scaling = DXGI_MODE_SCALING_UNSPECIFIED;
	swap_chain_desc.SampleDesc.Count = m_4xMSAA ? 4 : 1;
	swap_chain_desc.SampleDesc.Quality = m_4xMSAA ? (m_4xMSAA_quality - 1) : 0;
	...
	swap_chain_desc.OutputWindow = engine->get_window()->get_window_handle();
	swap_chain_desc.Windowed = true;
	swap_chain_desc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
	swap_chain_desc.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH;

	assert_if_failed(m_dxgi_factory->CreateSwapChain(m_command_queue.Get(), &swap_chain_desc, m_swap_chain.GetAddressOf()));
...
void Renderer::create_rtv_and_dsv_descriptor_heaps() {
	...
	assert_if_failed(m_d3d_device->CreateDescriptorHeap(&rtv_heap_desc, IID_PPV_ARGS(m_rtv_heap.GetAddressOf())));
	...
	assert_if_failed(m_d3d_device->CreateDescriptorHeap(&dsv_heap_desc, IID_PPV_ARGS(m_dsv_heap.GetAddressOf())));
}
void Renderer::flush_command_queue() {
	...
	assert_if_failed(m_command_queue->Signal(m_fence.Get(), m_current_fence));
	if (m_fence->GetCompletedValue() < m_current_fence) {
		HANDLE event_handle = CreateEventEx(nullptr, false, false, EVENT_ALL_ACCESS);
		assert_if_failed(m_fence->SetEventOnCompletion(m_current_fence, event_handle));
